...
class MetaAnalyzer:
    ...
    def check_setup_failure(self, path):
        ...
        if error_log is not None:
            # Orc3 setup should never appear in failure log, therefore entire log is parsed, without time filtering
    ...
    def check_teardown_failure(self, path):
        ...
        print(error_log)
        if error_log is not None:
            # Orc3 teardown fail should never appear in failure log, therefore entire log is parsed, without time filtering
    ...
    def check_quark_failure(self, path):
        ...
        print(error_log)
        if error_log is not None:
            if self.fuzzy_lookup_string("L0 test FAILED", error_log, 90) > 90: # ACPI script
                return True
            elif self.fuzzy_lookup_string("Quark test failed", error_log, 90) > 90: # Decode script
                return True
            elif self.fuzzy_lookup_string("gfxoff test FAILED", error_log, 90) > 90: # ACPI script
                return True
            elif self.fuzzy_lookup_string("gfxoff test inconclusive", error_log, 90) > 90: # ACPI script
                return True
            elif self.fuzzy_lookup_string("L0 test inconclusive", error_log, 90) > 90: # ACPI script
                return True
            elif self.fuzzy_lookup_string("L0 test inconclusive", error_log, 90) > 90: # ACPI script
                return True
            elif self.fuzzy_lookup_string("Could not find Quark.exe file on system", error_log, 90) > 90: # ACPI script
                return True
    ...
    def check_score_failed_to_read(self, path):
        ...
        error_log = self.get_file_path(["amd_orc3_errors.log"], path)
        info_log = self.get_file_path(["amd_orc3_info_only.log"], path)
        print(error_log)
        if error_log is not None:
    ...
    def check_for_unexpected_reboot(self, path):
        ...
        fail_log = self.get_file_path(["amd_orc3_errors.log"], path)
        if fail_log is not None:
