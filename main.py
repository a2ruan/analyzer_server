        ...
        if target_uuid != None:
            try:
                ...
                if 'result' in result:
                    ...
                    dbc.write_to_table_by_uuid(table="bkc_regression_test",uuid=target_uuid,payload={
                        "report":test_status,
...
def submit_job_by_uuid(uuid, request: Request = None):
    try:
        ...
        task_status_string[uuid] = "Querying orc3 database for test details"
        ...
        task_status_string[uuid] = "Copying files from GFXVAL to analyzer server"
            ...
            if test_details["test_log"] not in [None, "none", "None"]:
                ...
                if os.path.exists(dest_fpath) and ENABLE_CACHING:
                    ...
                    try:
                        ...
                        error_message = f"Error when copying files from GFXVAL due to network issues.  Please try again. {e}"
                        output_payload = {
                            "request": request,
                            "error_message": error_message
                        }
                ...
                task_status_string[uuid] = "Getting vbios configuration data from test logs"
                vbios_info = cachable_analyzer_helper.get_vbios_metadata(uuid) or {
                }
                if "uuid" in vbios_info:
                    vbios_info.pop("uuid")
                print("FW COMPONENTS")
                fw_components = {}
                try:
                    if "fw_components" in vbios_info:
                        fw_components = eval(vbios_info["fw_components"])
                        vbios_info.pop("fw_components")
...
async def get_table():
    ...
    output_dict = db_connector.fetch_data(table="bkc_regression_test", columns=[
