...
class database_connector:
    ...
    def get_tables(self, table="bkc_regression_test"):
        output = {}
        try:
    ...
    def fetch_data(self, table="bkc_regression_test", columns=["id","test_case","test_log","report","uuid","start_time","current_time","status","report_hash"]):
        output = {}
        try:
            DATABASE_URI = f"postgresql+psycopg2://{self.db_user_name}:{self.db_pwd}@{self.db_host}:{self.db_port}/{self.db_name}"
            engine = create_engine(DATABASE_URI)
...
if __name__ == "__main__":
    ...
    output_dict = db_connector.fetch_data(table="bkc_regression_test", columns=["id","test_case","test_log","uuid","start_time","current_time","status"])
    ...
    """
    db_connector = database_connector()
    db_connector.write_to_table_by_uuid(table="bkc_regression_test",uuid="a678c7c8-2a28-441e-97da-003fb7258100",payload={
        "report":"ok"})
    """
